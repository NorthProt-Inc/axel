...
describe('calculateDecayedImportance', () => {
	...
	describe('Type Multiplier Effects', () => {
		it('facts decay slowest, conversations decay fastest', () => {
			...
			const [fact, preference, insight, conversation] = results;
			expect(fact).toBeGreaterThan(preference as number);
			expect(preference).toBeGreaterThan(insight as number);
			expect(insight).toBeGreaterThan(conversation as number);
		});
	});
});
