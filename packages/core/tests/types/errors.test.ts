...
describe('Error types (ADR-020)', () => {
	...
	describe('ProviderError', () => {
		it('includes provider name and retryability', () => {
			const error = new ProviderError('Rate limit exceeded', 'anthropic', true);
		...
		it('can be non-retryable', () => {
			const error = new ProviderError('Invalid API key', 'anthropic', false);
			expect(error.isRetryable).toBe(false);
		});
	});
	...
	describe('ToolError', () => {
		...
		it('supports retryable tool errors', () => {
			const error = new ToolError('Temporary failure', 'api_call', true);
			expect(error.isRetryable).toBe(true);
		});
	});
