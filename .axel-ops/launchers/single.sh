#!/bin/bash
# .axel-ops/launchers/single.sh <division> [model] [budget]
# Manual test launcher for individual Division
# Usage: ./single.sh arch opus 5.00
#        ./single.sh research sonnet 3.00
...
DIV="${1:?Usage: single.sh <division> [model] [budget]}"
MODEL="${2:-opus}"
BUDGET="${3:-5.00}"
