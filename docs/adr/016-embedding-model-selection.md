...
2. pgvector에서 768d 벡터를 사용 중 (ADR-002)
3. 한국어 + 영어 혼용 환경
...
| Output dimensions | 3,072 (full) / 768 (truncated) / 256 (truncated) |
...
### Dimension Choice: 768d

| Dimension | Pros | Cons |
|-----------|------|------|
| 3,072 | 최고 정밀도 | 저장 공간 4x, 인덱스 빌드 느림, pgvector 메모리 증가 |
| **768** | axnmihn 호환 차원, 충분한 품질, IVFFlat 효율적 | full dimension 대비 소폭 품질 감소 |
| 256 | 최소 저장, 빠른 검색 | 정밀도 부족 (특히 유사한 기억 간 구별력) |

768d 선택 근거:
1. axnmihn과 동일한 차원이므로 pgvector 컬럼 타입 변경 불필요
2. 1,000 규모에서 3,072d의 추가 정밀도는 실질적 이점이 미미
3. Matryoshka 특성: 768d prefix는 3,072d 벡터의 유효한 부분집합
4. 향후 3,072d 업그레이드가 가능 (re-embed + ALTER COLUMN만 필요)

### Matryoshka Representation Learning

gemini-embedding-001은 Matryoshka 학습을 적용하여 벡터의 앞부분(prefix)이 낮은 차원에서도 유효하다:
- `vector[:768]`는 768d 독립 모델과 유사한 품질
- `vector[:256]`는 빠른 1차 필터에 사용 가능

이 특성으로 인해 dimension 변경이 re-train 없이 가능하다.
...
axnmihn의 `text-embedding-004` 벡터는 **gemini-embedding-001과 호환되지 않는다**. 동일 차원(768d)이지만 embedding space가 다르기 때문에 cosine similarity 결과가 의미 없다.
...
| Voyage voyage-3 | 67.13 | 1024 | $0.06/1M | 가격 우수 | 768d 미지원 (dimension mismatch), 생태계 작음 |
...
- MTEB #1 모델로 최고 품질의 벡터 검색 가능
- Matryoshka 지원으로 향후 dimension 유연하게 조정 가능
- Gemini API는 이미 Intent Classifier (Flash)와 Session Summary에 사용 — 추가 API key 불필요
- 768d로 axnmihn IVFFlat 인덱스 설정(lists=100) 재사용 가능
